...
Kadira.connect = function(appId, appSecret, options) {
  ...
  logger('Starting app: ', appId);
  console.log('Kadira: Successfully starting');
...
Kadira._sendAppStats = function () {
  ...
  Kadira.send({
    ...
    if(err)
      console.trace('Kadira Error on sending appStats:', err.message);
  });
}
